...
class AccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        for label, pred_label in zip(labels, preds):  # zip(): 将列表打包成可迭代的元组
            if pred_label.shape != label.shape:
                pred_label = mx.ndarray.argmax(pred_label, axis=self.axis)  # 返回沿轴axis最大值的索引
...
def train_net(args):
    ...
    args.num_layers = int(args.network[1:])  # 网络层数
    print('num_layers', args.num_layers)
    if args.per_batch_size == 0:
        args.per_batch_size = 128
    args.batch_size = args.per_batch_size * args.ctx_num  # 每次迭代批量大小 x GPU核心数
